...
- **Keystatic ships to production** (`docs/DECISIONS.md` #25): the editor at `/keystatic` runs on the live site in GitHub storage mode — sign in with GitHub, saving commits to the repo via its API, the site rebuilds. Exactly two routes render on demand (`/keystatic`, `/api/keystatic/*`); every reading page stays prerendered and free of framework JS. Credentials live in `.env` locally and Cloudflare secrets in production, never in the repo (see `.env.example`).
